...
export default function QRCodeManager({ qrToken, orgName, logoUrl = "", canRegenerate = true }: { qrToken: string; orgName: string; logoUrl?: string; canRegenerate?: boolean }) {
  ...
  return <div className="flex max-w-sm flex-col items-center gap-5 rounded-xl border border-border bg-surface p-6">
    <div ref={containerRef} className="rounded-xl border border-border bg-white p-4"><QRCodeCanvas value={publicUrl} size={220} level="H" includeMargin imageSettings={logoUrl ? { src: logoUrl, height: 48, width: 48, excavate: true } : undefined} /></div>
