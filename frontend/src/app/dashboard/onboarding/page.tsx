    ...
    <div className="mx-auto max-w-3xl space-y-8 pb-16">
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-subtle sm:p-8">
        ...
        {step === 2 && <div className="space-y-6">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start"><CloudinaryImageUploader value={logo} onChange={setLogo} disabled={role !== "BOSS"} /><label className="flex-1 text-sm font-medium text-ink">Nom de lՎtablissement<input value={name} onChange={(e) => setName(e.target.value)} className="mt-2 w-full rounded-lg border border-border bg-canvas px-3 py-3 outline-none focus:border-ink" /></label></div>
