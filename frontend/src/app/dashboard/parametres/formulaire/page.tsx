...
export default function FormulairePage() {
  ...
  const makeDefault = async (form: FormTemplate) => { setDefaultingId(form.id); setActionError(""); try { const response = await apiClient.patch<FormTemplate>(`/form-templates/${form.id}/`, { is_default: true, is_active: true }); setForms((items) => items.map((item) => ({ ...item, is_default: item.id === form.id }))); setSelected(response.data); } catch { setActionError("Impossible de définir ce formulaire par défaut. Réessaie."); } finally { setDefaultingId(null); } };
