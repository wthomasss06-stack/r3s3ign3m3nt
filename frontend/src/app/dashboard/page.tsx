...
export default function RegistrePage() {
  ...
  const load = useCallback(async (initial = false) => {
    ...
    try {
      ...
      setSchema(tpl.data.fields_schema); setRecords(chk.data.results); setTotalCount(chk.data.count); setOrgName(org.data.name); setStats(summary.data); setUser(me.data); setAvatarFailed(false); setLastUpdated(new Date()); setState("ready");
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-bold text-ink">Registre</h1><p className="text-sm text-ink-soft">{stats?.total ?? totalCount} visiteur{(stats?.total ?? totalCount) > 1 ? "s" : ""} · actualisation automatique toutes les 30 secondes{lastUpdated ? ` · ${lastUpdated.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}` : ""}</p></div><div className="flex flex-wrap items-center justify-end gap-3"><div className="flex max-w-full items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2"><div className="grid h-8 w-8 shrink-0 place-items-center overflow-hidden rounded-full bg-canvas text-xs font-bold text-ink">{user?.avatar_url && !avatarFailed ? <img src={user.avatar_url} alt={`Photo de ${user.full_name || user.email}`} className="h-full w-full object-cover" referrerPolicy="no-referrer" onError={() => setAvatarFailed(true)} /> : (user?.full_name || user?.email || "?").slice(0, 1).toUpperCase()}</div><div className="min-w-0"><p className="max-w-[150px] truncate text-xs font-semibold text-ink">{user?.full_name || user?.email}</p><p className="text-[11px] text-ink-soft">{user?.role === "BOSS" ? "Patron" : user?.role === "GERANT" ? "Gérant" : "Staff"}</p></div></div>{canExport && <button onClick={() => void handleExport()} disabled={totalCount === 0 || exporting} className="rounded-full bg-cta px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50">{exporting ? "Export en cours…" : "Exporter en CSV"}</button>}</div></div>
